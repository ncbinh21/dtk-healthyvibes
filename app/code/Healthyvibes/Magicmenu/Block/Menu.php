...
namespace Healthyvibes\Magicmenu\Block;
...
class Menu extends \Magento\Catalog\Block\Navigation
{
    ...
    /**
     * @var \Healthyvibes\Magicmenu\Helper\Data
     */
    public $_helper;
    ...
    public function __construct(
        ...
        if (is_null($configModule['vmenu']['notIncludeNav'])) {
            $configModule['vmenu']['notIncludeNav'] = '';
        }
    ...
    public function drawMainMenu()
    {
        ...
        foreach ($categories as $catTop) {
            ...
            if ($isDropdown) {
                ...
                $menu = ['desktop' => $childHtml, 'mobile' => $childHtml];
            } else { // Draw Mega Menu
                $idTop    = $catTop->getEntityId();
                $data     = isset($this->extData[$idTop]) ? $this->extData[$idTop] : '';
                $blocks   = ['top'=>'', 'left'=>'', 'right'=>'', 'bottom'=>''];
                if ($data) {
                    ...
                    $remove = ['top'=>'', 'left'=>'', 'right'=>'', 'bottom'=>'', 'cat_id'=>''];
    ...
    public function drawExtraMenu()
    {
        ...
        if ($count) {
            ...
            foreach ($extMenu as $ext) {
                ...
                if ($link) {
                    $drawExtraMenu .= '<a class="level-top" href="' . $url . '"><span>' . $ext->getName() . $this->getCatLabel($ext) . '</span></a>';
                } else {
                    $drawExtraMenu .= '<span class="level-top"><span>' . $ext->getName() . $this->getCatLabel($ext) . '</span></span>';
                }
                if ($html) {
                    $drawExtraMenu .= $html;
                } //$drawExtraMenu .= '<div class="level-top-mega">'.$html.'</div>';
                $drawExtraMenu .= '</li>';
                $i++;
                $class = ($i == $count) ? 'last' : '';
            }
    ...
    public function getExtraMenu()
    {
        ...
        $collection = $this->_magicmenuCollectionFactory->create()
                        ->addFieldToSelect(['link','name', 'cat_col', 'magic_label','ext_content','order'])
    ...
    public function getCatLabel($cat)
    {
        ...
        foreach ($label as $lab) {
            if ($lab) {
                $html .= '<span class="cat_label ' . $lab . '" rel=' . __(trim($lab)) . '></span>';
            }
        }
        return $html;
    }
    ...
    {
        $url = '';
        $image = $this->_dirMedia . 'healthyvibes/magicmenu/images/' . $category->getEntityId() . '.png';
        if (file_exists($image)) {
            $url = $this->_urlMedia . 'healthyvibes/magicmenu/images/' . $category->getEntityId() . '.png';
        }
        if ($url) {
            return '<a class="a-image" href="' . $category->getUrl() . '"><img class="img-responsive" alt="' . $category->getName() . '" src="' . $url . '"></a>';
        }
    }
    ...
    {
        $url = '';
        $image = $this->_dirMedia . 'healthyvibes/magicmenu/thumbnail/' . $category->getEntityId() . '.png';
        if (file_exists($image)) {
            $url = $this->_urlMedia . 'healthyvibes/magicmenu/thumbnail/' . $category->getEntityId() . '.png';
        }
        if ($url) {
